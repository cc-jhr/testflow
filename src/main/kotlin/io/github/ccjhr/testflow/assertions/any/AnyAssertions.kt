 ...
 * @see mustNotBeOfType
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustBeOfType(type: KClass<*>) {
    ensureNotNull(this.content)
    if (!type.isInstance(this.content)) {
        fail("Expecting <${this.content}> to be of type <${type.qualifiedName}>, but is <${this.content!!::class.qualifiedName}>.")
    }
}
 ...
 * @see mustBeOfType
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustNotBeOfType(type: KClass<*>) {
    ensureNotNull(this.content)
    if (type.isInstance(this.content)) {
 ...
 * @see isNotEqualTo
 */
inline infix fun <reified T: Any?> AssertionContext<T>.isEqualTo(other: Any) {
    ensureNotNull(this.content)
 ...
 * @see isEqualTo
 */
inline infix fun <reified T: Any?> AssertionContext<T>.isNotEqualTo(other: Any) {
    ensureNotNull(this.content)
    if (this.content == other) {
 ...
 * @see mustNotBeSameAs
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustBeSameAs(other: Any) {
    ensureNotNull(this.content)
    if (this.content !== other) {
 ...
 * @see mustBeSameAs
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustNotBeSameAs(other: Any) {
    ensureNotNull(this.content)
    if (this.content === other) {
 ...
 * @see mustBe
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustNotBe(adjective: AnyAssertionAdjective) {
    when(adjective) {
 ...
 * @see mustNotBe
 */
inline infix fun <reified T: Any?> AssertionContext<T>.mustBe(adjective: AnyAssertionAdjective) {
    when(adjective) {
...
fun Any.referenceString() = "${this.javaClass.kotlin.qualifiedName}@${System.identityHashCode(this)}"
